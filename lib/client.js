window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function bridgeErrorMessage(result) {
      const detail = typeof result?.message === "string" ? result.message : "";
      const code = result?.code;
      if (code === "settings-not-exposed") {
        return (
          "dsh-free-search 未暴露 free-search 设置命名空间（settings-not-exposed）。" +
          "这通常是 dsh-free-search 0.4.22 需要 Harness 运行时 >= 0.1.2-alpha.2" +
          "（例如 DSH Desktop 0.7.1 内置仅 0.1.2-alpha.1）。" +
          "请升级 DSH Desktop，或按 README 的 Compatibility 一节处理。"
        );
      }
      return detail ? `切换失败：${detail}` : "切换失败：请稍后重试";
    }
    ...
    async function currentEngine() {
      try {
        ...
        if (result.ok !== true) {
          return { engine: DEFAULT_ENGINE, warn: bridgeErrorMessage(result) };
        }
        const view = result.value?.namespaces?.find((n) => n.ns === FREE_SEARCH_NS);
        if (view === undefined) {
          return {
            engine: DEFAULT_ENGINE,
            warn:
              "免费搜索设置桥接在线，但 free-search 命名空间未暴露（namespaces 为空）。" +
              "dsh-free-search 0.4.22 需要 Harness 运行时 >= 0.1.2-alpha.2 才能注册设置分区；" +
              "请升级 DSH Desktop，或按 README 的 Compatibility 一节处理。",
          };
        }
        return { engine: view.value?.provider ?? DEFAULT_ENGINE, warn: null };
      } catch {
        // 桥接不可达：打开菜单时由 choose() 如实报错
        return { engine: DEFAULT_ENGINE, warn: null };
      }
    }
    //#endregion
    ...
    function SearchSwitcher({ locked = false }) {
      ...
      const [warn, setWarn] = react.useState(null); // 桥接级诊断（命名空间未暴露等）
      ...
      const choose = async (engineId) => {
        ...
        try {
          ...
          if (result.ok !== true) {
            setStatus("error");
            setError(bridgeErrorMessage(result));
            return;
          }
        ...
        children: [
          ...
          open && react_jsx_runtime.jsxs("div", {
            ...
            children: [
              ...
              status === "idle" && warn !== null && react_jsx_runtime.jsx("div", {
                className: "dshss-error",
                children: String(warn),
              }),
              status !== "loading" && react_jsx_runtime.jsx("div", {
